...
contract PostRegistry {
     ...
     *
     * @dev A fresh account starts at 1, not at a whole vote. That is the whole
     *      sybil defence, and it is the only one left now that vote weight is
     *      karma rather than distance from a seed set: every account can still
     *      vote on day one, but a stranger's vote is worth one per cent of a
     *      vote, and it takes a hundred of them to move a feed the way one
     *      trusted account does. A ring has to spend a hundred wallets per unit
     *      of influence instead of the two it used to.
     *
