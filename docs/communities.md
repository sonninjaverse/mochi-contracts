...
resetting the existing post cursor. Already-cached posts are classified from
their text and original event position. Keep the existing SQLite database.

Publish the updated `infra/rewrite-profile.js` with the frontend: `/m/`,
`/m/<name>/`, `/subs/` and their Next.js navigation payloads need its mappings.
...
into an in-memory SQLite database, and tests the exported site on desktop and
