...
contract NetVotesFeed {
        ...
        returns (uint256[] memory orderedIds, uint256[] memory scores)
    {
        ...
        for (uint256 i; i < candidateIds.length; ++i) {
            ...
            // weightedLikes and weightedDislikes are already filtered by the
            // social graph: a vote from an account no one reachable follows
            // counts zero. Using the raw counts instead is what a sybil ring
            // is built to exploit.
