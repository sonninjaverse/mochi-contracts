...
/// @dev Which community a post is in is not stored in contract state.
///      A post names its community in its own text, which already lives in
///      event data, and no ranking contract reads it — so by this repository's
///      storage rule it does not belong in storage. What is left is the only
///      part that needs a single source of truth: who holds a name, and who
///      has joined.
contract CommunityRegistry {
    ...
    ///      the way SocialGraph.followedAt does.
    ...
    /// @dev Validated on chain, unlike a handle, because a community name is
    ///      recovered from free text by the indexer. Exactly one spelling may
    ///      ever match, or `m/VietNam` and `m/vietnam` become two communities
    ///      that look like one.
